getStandings = function( data ) {
	...
	$.each( parsedData, function( divisionName, divisionArray ) {
		...
		$.each( divisionArray, function( j, teamArray ) {
			var img = new Image();
			img.src = "http://52.7.194.101:4002/Logos/TeamIcons_" + teamArray["nm"] + ".png";
			
			table += "<tr class='" + teamArray["name"].replace(/\s/g, '') + "'>";
			table += "<td><img src='http://52.7.194.101:4002/Logos/TeamIcons_" + teamArray["nm"] + ".png'></td>";
...
standings = function( division ) 
{
	$(document).ready(function() {
		$.ajax({
			url: "http://52.7.194.101:4002/Web/Standings",
			jsonp: "callback",
			dataType : "jsonp",
